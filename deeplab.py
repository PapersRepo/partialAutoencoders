...
def assp_branch(in_channels, out_channels, kernel_size, dilation):
    """
...
class DeepLab(BaseModel):
    ...
    def forward(self, x):
        """
        In this forward pass two modifications have been added:
        - (H,W)=(x.size(2),x.size(3)) -> (H,W)=(x.size(2)//2,x.size(3)//2)
        - x = torch.tanh(x) at the end 
        """
        H, W = int(x.size(2) // 2), int(x.size(3) // 2)
